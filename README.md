...
после успешного коммита транзакции через `transaction.on_commit(...)`.

## 2. Основной функционал
- CRUD для постов через REST API
- Кеширование детального эндпоинта `GET /api/v1/posts/{id}/`
- Инвалидация кеша при `PATCH`, `PUT` и `DELETE` после успешного коммита БД
...
Если одновременно подняты `dev` и `prod`, продакшен-версия будет доступна на
`http://127.0.0.1:8001/`.
...
- При одновременном запуске `dev` и `prod` продакшен-API доступен по адресу
...
- При изменении или удалении поста кеш инвалидируется через
  `transaction.on_commit(...)` только после успешного коммита транзакции БД
...
удалении поста. Дополнительно сброс кеша привязан к `transaction.on_commit(...)`,
поэтому Redis обновляется только после успешного завершения транзакции в БД.
...
- Для продакшен-стека при одновременном запуске используется порт `8001`
